...
class Screensaver(xbmcgui.WindowXMLDialog):
    ...
    def _grab_screenshot(self, path):
        if not (os.path.isfile(path) and os.path.isfile('/usr/local/bin/ffprobe') and os.path.isfile('/usr/local/bin/ffmpeg')):
            return [None, None]
        try:
            proc = subprocess.Popen(['/usr/local/bin/ffprobe', '-v', 'error', '-show_entries', 'format=duration', '-of', 'default=noprint_wrappers=1:nokey=1', path], stdout=subprocess.PIPE)
            duration = int(float(proc.communicate()[0].strip()))
            screenshot_path = os.path.join('/tmp', 'kodi_' + uuid.uuid4().hex + '.jpg')
            screenshot_seconds = random.randint(0, int(0.9 * duration))
            subprocess.check_call(['/usr/local/bin/ffmpeg', '-y', '-ss', str(screenshot_seconds), '-i', path, '-vframes', '1', '-f', 'image2', screenshot_path, '-v', 'quiet'])
